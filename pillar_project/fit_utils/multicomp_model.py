...
class MulticomponentCalibrationModel:
    ...
    def _propose_Gamma_update(self, scores, component_posteriors, component_num):
        ...
        """
        skewness_i, loc_i, scale_i = self.get_component_params(component_num)
        v, w = MulticomponentCalibrationModel.get_truncated_normal_moments(scores, (skewness_i, loc_i, scale_i))
        g = (
    ...
    def get_truncated_normal_moments(observations, component_params):
        ...
        v, w = MulticomponentCalibrationModel.trunc_norm_moments(truncated_normal_loc, truncated_normal_scale)
        return v, w
    ...
    def get_sample_density(self, scores, sampleNum):
        ...
        """
        density = np.array([self.sample_weights[sampleNum,compNum] * self.get_component_density(scores, compNum) \
                          for compNum in range(self.num_components) if self.sample_weights[sampleNum,compNum]]).sum(axis=0)
        return density
    ...
    def get_sample_cdf(self,scores, sampleNum):
        ...
        if sampleNum >= self.sample_weights.shape[0] or sampleNum < 0:
            raise ValueError(f"Sample number {sampleNum} must be greater than 0 and less than the number of samples {self.sample_weights.shape[0]}.")
    ...
    def get_component_cdf(self, scores, componentNum):
        ...
        if componentNum >= self.num_components or componentNum < 0:
            raise ValueError(f"Component number {componentNum} must be greater than 0 and less than the number of components {self.num_components}.")
    ...
    def _sample_component_posteriors(self, sample_scores : np.array, sample_weights : np.array,**kwargs) -> np.array:
        ...
        assert np.allclose(np.sum(comp_posteriors, axis=1), 1), f"Posterior probabilities must sum to 1.\n{comp_posteriors.sum(axis=1)}"
        return comp_posteriors
